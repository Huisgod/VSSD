...
/*static void push_hash_table(iobuf *inser_p)
{
	iobuf *choose_hash_head = hash_head[inser_p->blkno % HASH_VALUE];
	if(choose_hash_head == NULL) 
	{
		inser_p->h_next = NULL;
		inser_p->h_prev = NULL;
		hash_head[inser_p->blkno % HASH_VALUE] = inser_p;
	}
	else if(choose_hash_head->blkno > inser_p->blkno)
	{
		inser_p->h_next = hash_head[inser_p->blkno % HASH_VALUE];
		inser_p->h_prev = NULL;
		hash_head[inser_p->blkno % HASH_VALUE]->h_prev = inser_p;
		hash_head[inser_p->blkno % HASH_VALUE] = inser_p;
	}
	else
	{
		iobuf *ptr = hash_head[inser_p->blkno % HASH_VALUE];
		while((ptr->h_next!=NULL) && (ptr->h_next->blkno < inser_p->blkno))
		ptr = ptr->h_next;

		inser_p->h_prev = ptr;
		inser_p->h_next = ptr->h_next;
		ptr->h_next = inser_p;
		if(ptr->h_next != NULL)
			ptr->h_next->h_prev = inser_p;
	}
	
}

static iobuf *get_hash_table(int get_v)
{
	iobuf *ptr = hash_head[get_v % HASH_VALUE];
	if(ptr == NULL)return NULL;
	while((ptr->h_next!=NULL) && (ptr->h_next->blkno < get_v))
	ptr = ptr->h_next;
...
static iobuf * ioqueue_get_request_from_lbn_scan_queue (subqueue *queue)
{
   ...
   //temp = get_hash_table(queue->lastblkno);
   //if(temp == NULL)
...
//****************jian_add****************************
/*	if(temp->h_next != NULL)
		temp->h_next->h_prev =  temp->h_prev;
	if(temp->h_prev != NULL)
		temp->h_prev->h_next =  temp->h_next;
	if(temp->h_next ==NULL)
	{
		hash_head[temp->blkno % HASH_VALUE] =  NULL;
	}
*/
//****************jian_end****************************
