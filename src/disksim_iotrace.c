...
{
   char line[201];
   user_time_state *ptr;//*jian add save user's infor

   if (fgets(line, 200, tracefile) == NULL) {
      addtoextraq((event *) new);
      return(NULL);
   }
   if (sscanf(line, "%lf %d %d %d %x %d\n", &new->time, &new->devno, &new->blkno, &new->bcount, &new->flags ,&new->user_id) != 6) {
	//*jian =====user allocating or deallocating commant come in===== start 2011/11/24
	//*jian allocating format:  (allocating?/deallocating? 0/1) (userid):(allocating space<bytes>) (allocating bandwidth<%>)

	double time;
	int allocating_flag;//allocating=0 ;deallocating=1
	int user_id;
	int user_init_space;
	int user_request_log_space;//(blocks)
	int user_request_real_space;//(blocks)
	double user_request_bandwidth;

   	if (sscanf(line, "%lf %d %d:%d %d %d %lf\n",\
	&time, &allocating_flag, &user_id, &user_init_space, &user_request_log_space, &user_request_real_space, &user_request_bandwidth) != 7) 
	{
		fprintf(stderr, "Wrong number of arguments for I/O trace event type\n");
		fprintf(stderr, "line: %s", line);
		ddbg_assert(0);
	...
	{
	//	fprintf(outputfile, "j_debug user_id = %d\tuser_request_space = %d\tuser_request_bandwidth =%d\n"\
			, user_id, user_request_space, user_request_bandwidth);
		if(allocating_flag == 0)//*jian if user want allocating space
		{

			//*jian check whether have idle spaces and bandwidths-end
			ptr=find_user(user_id);
		        if(ptr==NULL)
			{
				ioreq_event *new_t = getfromextraq();
				new_t->type = USER_ALLOCATING;

				new_t->time = time;				//	1
				new_t->user_id = user_id;			//	2
				new_t->flags   = allocating_flag;		//	3
				new_t->blkno   = user_init_space;		//	4
				new_t->tempint1 = user_request_log_space;	//	5
				new_t->tempint2 = user_request_real_space;	//	6
				new_t->start_time = user_request_bandwidth;	//	7
				...
				add_user(user_id, user_init_space, user_request_log_space, user_request_real_space, user_request_bandwidth);
			}
			else
			{
				fprintf(stderr, "The user_id has been allocating!\n");
                		fprintf(stderr, "line: %s", line);
                		ddbg_assert(0);
			}
		}
		else if(allocating_flag == 1)
		{
		#ifdef CURREN_BLOCK_ENABLE
		#ifdef DEALLOCATING_ENABLE
			ptr=find_user(user_id);
			if(ptr==NULL)
			{
				fprintf(stderr,"deallocating error\n");
                		fprintf(stderr, "line: %s", line);
				ASSERT(0);
			}
			else
		...
		return iotrace_ascii_get_ioreq_event (tracefile,new);
	}
	//*jian =====user allocating or deallocating commant come in===== end 2011/11/24
   }
   if (new->flags & ASYNCHRONOUS) {
	...
	//*jian user allocating "new space" and "new bandwidth"
        ptr=find_user(new->user_id);
        if(ptr==NULL)
        {
		fprintf(stderr, "The trace non allocating.(the user_id can't find)\n");
		fprintf(stderr, "line: %s\n", line);
                ddbg_assert(0);
        }
	//*jian user deallocating ("space"+"bandwidth")
