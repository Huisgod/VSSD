...
typedef struct _vssd_user_time_state {
	...
	int free_pool_block_cnt;        // hu add for record remain block
	...
	#endif
	//hu add for borrow block
	int dead_block_cnt; //how many dead block borrow from other user
	int free_block_cnt;
	...
	unsigned long live_page_cnt;    //hu
	unsigned long live_page_per_sec;//hu
	#ifdef DEBUG_VSSD	
	unsigned long using_pages_cnt;		//*jian the user's using block count (ssd's block)
	unsigned long using_pages_per_sec;	
	#endif
	...
	//hu add for GC cost estimate
	unsigned int gc_live_page_copy_per_sec;
	unsigned gc_issues_per_sec;	
