...
typedef struct _vssd_user_time_state {
	...
	int user_id;
	int request_cnt;//*jian complete request cnt
	double time;
}user_time_state;

user_time_state *user_time;
...
typedef struct _block_metadata {
    ...
    int		user_id;		//*jian check
    #ifdef DEBUG_VSSD
    int 	*page_user_id;		//*jian check
    #endif
    unsigned int    bsn[MAX_USERS_NUMBER];                // block sequence number (version number for blocks) //*jian modify
} block_metadata;
...
typedef struct _plane_metadata {
    ...
    unsigned int user_allocating_blocks[MAX_USERS_NUMBER];   //*jian  total number of free block in the elem
    int user_using_blocks[MAX_USERS_NUMBER];   //*jian  total number of free block in the elem
    #endif
    #endif
    ...
    unsigned int active_page[MAX_USERS_NUMBER];       // this points to the next page to write inside an		//*jian -note next allocating block
                                    // active block.
    ...
    int user_id;
    #endif
    int		free_pages[MAX_USERS_NUMBER]; //*jian 0=non-allocating pages ;1=user_1 ;2 = user2 ...
    int		using_nonallocating_pages[MAX_USERS_NUMBER];
} plane_metadata;
...
//    int *lba_table;                 // a table mapping the lba to the physical pages
    ...
    unsigned int active_page[MAX_USERS_NUMBER];   // this points to the next page to write inside an		//*jian -note current block
                                    // active block  .						//*jian modify active_page -> active_page[10]
    int		user_id;	    //*jian modify add current user using
    int		user_id_2;	    //*jian show GC total time
    ...
    int plane_to_write[MAX_USERS_NUMBER];             // which plane to write next?
    ...
    #ifdef CURREN_BLOCK_ENABLE
    #ifdef GC_SELF_BLOCKS
    unsigned int user_using_blocks[MAX_USERS_NUMBER];   //*jian  total number of free block in the elem
    #endif
    #endif
    ...
    #endif
    int		element_number;
    double	simtime_for_quanta_base_schedule[MAX_USERS_NUMBER];
	//****************************jian add***********************************
...
//#define SSD_DATA_BYTES_PER_SECTOR               512	//*jian move to src/disksim_global.h
...
double  _ssd_write_page_osr(ssd_t *s, ssd_element_metadata *metadata, int lpn, int user_id);
