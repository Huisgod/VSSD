...
void ssd_update_free_block_status(int blk, int plane_num, ssd_element_metadata *metadata, ssd_t *s)
{
    ...
    if(user_id != 0)
    {
		...
		#ifdef BORROW_DEAD_BLOCK
		if(user_id != metadata->user_id) 
		{		
			metadata->plane_meta[plane_num].user_allocating_blocks[metadata->user_id]++;
		}
		metadata->user_using_blocks[user_id]--;
	    metadata->plane_meta[plane_num].user_using_blocks[user_id]--;	
				
		#endif
		#ifndef BORROW_DEAD_BLOCK
	    metadata->user_using_blocks[user_id]--;
	    metadata->plane_meta[plane_num].user_using_blocks[user_id]--;	
		#endif
...
static int ssd_pick_block_to_clean2(int plane_num, int elem_num, double *mcost, ssd_element_metadata *metadata, ssd_t *s)
{
	...
	int min_valid_global = s->params.pages_per_block - 1; //global min value    	
    listnode *greedy_list;
	user_time_state *ptr;
	user_time_state *ptr2;	
    *mcost = 0;
	...
	#ifdef BORROW_DEAD_BLOCK
	listnode *greedy_list_global;
	ll_create(&greedy_list_global);
	#endif
    for (i = 0; i < s->params.blocks_per_element; i ++) {
        if (_ssd_pick_block_to_clean(i, plane_num, elem_num, metadata, s)) { //check if this block is belong to this user
			
			#ifdef BORROW_DEAD_BLOCK
			//mantain global greedy list
			if (metadata->block_usage[i].num_valid <= min_valid_global) {
                ASSERT(i == metadata->block_usage[i].block_num);
                ll_insert_at_head(greedy_list_global, (void*)&metadata->block_usage[i]); //it'a s global greedy list if disable GC_self block
                min_valid_global = metadata->block_usage[i].num_valid;
                block_global = i;							
			}
			#endif
			
            // greedily select the block //mantain self greedy list
            if (metadata->block_usage[i].num_valid <= min_valid && metadata->block_usage[i].user_id == metadata->user_id) {
                ASSERT(i == metadata->block_usage[i].block_num);
                ll_insert_at_head(greedy_list, (void*)&metadata->block_usage[i]); //it'a s self greedy list if disable GC_self block
...
#ifdef BORROW_DEAD_BLOCK
ptr= find_user(metadata->user_id);
if(min_valid-5 > min_valid_global && ptr->dead_block_cnt <=1000 && min_valid_global == 0) //aleast one dead block can be borrowed 
{
	assert(min_valid_global==0); //must be dead block
	size_global = ll_get_size(greedy_list_global);
	for(i=0;i<size_global;i++)
	{
		block_metadata *bm_global;
		int mig_blk_global;

		listnode *N = ll_get_nth_node(greedy_list_global,i);
		bm_global = ((block_metadata *)N->data);
		
		if(i == 0){
			ASSERT(min_valid_global == bm_global->num_valid);		
		}
	        // this is the last of the greedily picked blocks.
        if (i == size_global -1) {
            // select it!
            block_global = bm_global->block_num;
			if(bm_global->user_id != metadata->user_id)
			{
				ptr->dead_block_cnt++;
			}
            break;
...
#if MIGRATE
            // migration
        mig_blk_global=ssd_pick_wear_aware_with_migration(bm_global->block_num, bm_global->rem_lifetime, avg_lifetime, mcost, bm_global->plane_num, elem_num, s);
            if (mig_blk_global != bm_global->block_num) {
                // data has been migrated and we have a new
                // block to use
                block_global = mig_blk_global;
				if(bm_global->user_id != metadata->user_id)
				{
					ptr->dead_block_cnt++;
				}
            ...
            if (ssd_pick_wear_aware(bm_global->block_num, bm_global->rem_lifetime, avg_lifetime, s)) {
                block_global = bm_global->block_num;
				if(bm_global->user_id != metadata->user_id)
				{
					ptr->dead_block_cnt++;
	...
	assert(block_global != -1);
}//end of if
ll_release(greedy_list_global);
if(block_global!=-1)
return block_global;
#endif
...
static double _ssd_clean_block_partially(int plane_num, int elem_num, ssd_t *s)
{
	...
	for (i = 0; i < (s->params.pages_per_block-1); i ++) {
		user_time_state *ptr2;
		ptr2 = find_user(metadata->block_usage[block].page_user_id[i]);
		ptr2->using_pages_cnt --;
		//ptr->using_pages_per_sec--; //hu
